import { Injectable, NotImplementedException, Logger, InternalServerErrorException, NotFoundException, HttpException, HttpStatus } from '@nestjs/common';
import { Repository } from 'typeorm';
import { Idea } from './models/idea.model';
import { InjectRepository } from '@nestjs/typeorm';
import { IdeaRequestDto } from './dto/idea-request.dto';
import { IdeaResponseDto } from './dto/idea-response.dto';
import { BaseResponseDto } from 'src/shared/base-response.dto';
...
export class IdeaService {
    ...
    async updateIdea(data: Partial<IdeaRequestDto>, id: string): Promise<any> {
        ...
        if (!idea) throw new HttpException(`idea with id ${id} doesn't exist`, HttpStatus.NOT_FOUND);
        let result = await this.ideaRepository.update({ id }, data);
        if (!result) throw new HttpException('Failed to update idea', HttpStatus.NOT_MODIFIED);
        Logger.debug(result);
    }
